...
class SSTVDecoder(object):
    ...
    def __init__(self, audio_file, language='English'):
        """
    ...
    def _decode_vis(self, vis_start):
        ...
        if not parity:
            raise ValueError("Error decoding VIS header (invalid parity bit)" if self.language == 'English' else "Eroare la decodificarea antetului VIS (bit de paritate invalid)")
        ...
        if vis_value not in spec.VIS_MAP:
            error = "SSTV mode is unsupported (VIS: {})" if self.language == 'English' else "Mod SSTV nesuportat (VIS: {})"
    ...
    def _decode_image_data(self, image_start):
        ...
        # Use list comprehension to initialize list so we can return data early
    ...
    def _draw_image(self, image_data):
        ...
        log_message("Drawing image data..." if self.language == 'English' else "Desenarea datelor imaginii...")
        ...
        log_message("...Done!" if self.language == 'English' else "...Gata!")
        return image
